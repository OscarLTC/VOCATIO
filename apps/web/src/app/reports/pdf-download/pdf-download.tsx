...
export function PdfDownload(props: PdfDownloadProps) {
  ...
  useEffect(() => {
    const getSurveyPersonData = async () => {
      await axios
        .get(`${environment.apiUrl}/surveyEnterprisePerson/${props.idPDF}`)
        .then((res) => {
    ...
    <>
      {imageURL && (
        <PDFDownloadLink
          document={
            <Document style={{ height: '100%' }}>
              ...
              <Page size="A4" style={styles.page}>
                ...
                <View>
                  ...
                  >
                    ...
                    <Text style={{ marginTop: '20px' }}>
                      ...
                      <Text style={{ fontWeight: 'bold' }}>modelo V.A.K.</Text>{' '}
                      son:
                    </Text>
                    <View
                      style={{
                ...
                >
                  <View
                    style={{ width: '40px', marginLeft: 'auto', opacity: 0.1 }}
                  >
                ...
                >
                  <View>
                    ...
                    <Text style={{ marginTop: '10px', lineHeight: '1.8' }}>
                      Luego del procesamiento de los datos brindados se obtiene
                      como estilo de aprendizaje predominante:
                    </Text>
                    <View
                      style={{
                        marginTop: '15px',
                    ...
                    >
                      {surveyPerson?.result.mainFeatures
                        ...
                        .map((feature, index) => {
                          if (
                            index !==
                            surveyPerson?.result.mainFeatures.split('.')
                              .length -
                              1
                          ) {
                            return (
                              <Text key={index}>{`o ${feature}. \n`}</Text>
                            );
                          }
                        })}
                    </Text>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        “Conocer tu estilo de aprendizaje te ayudará a
                        identificar el método de estudio más adecuado para ti.
                        De esta forma estarás más cerca a conseguir tus metas
                        académicas y tu éxito personal.”
                      </Text>
                    </View>
                  </View>
                  <View
                  ...
                  >
                    <Text>
                      Si tienes alguna duda o consulta sobre tus resultados
                      puedes escribirnos a:{' '}
                      <Link src="#">gerencia@vocatio.pe</Link>, con gusto un
                      profesional especializado te atenderá.
                    </Text>
                  </View>
                </View>
                <View
          ...
          fileName={`${surveyPerson?.survey_enterprise.name}-${surveyPerson?.person.name}_${surveyPerson?.person.lastName}-${surveyPerson?.survey_enterprise.survey.name}`}
        >
          {({ blob, url, loading, error }) =>
            loading ? (
              <div className="animate-spin w-fit mx-auto  m-1">
                <RiLoader4Fill size={20} />
              </div>
            ) : surveyPerson?.id ? (
              <div className=" w-fit mx-auto m-1  rounded text-sm text-blue-500 underline">
                {`${environment.surveyDomain}/reportes/pdf/${surveyPerson?.id}`}
              </div>
            ) : (
              <div className="animate-spin w-fit mx-auto  m-1">
                <RiLoader4Fill size={20} />
      ...
      <div className="absolute overflow-hidden w-[1000px]">
        <Doughnut
          className="absolute "
          ref={chartRef}
          width={100}
